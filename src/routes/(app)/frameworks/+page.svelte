<script lang="ts">
  ...
  let { data } = $props<{ data: { seededInstances: FrameworkSeedInstance[]; workspaceId: number; projectId: number; currentUser?: { displayName?: string } } }>();

  let projectId = $derived(page.url.searchParams.get('project') ?? String(data.projectId));
  let storageKey = $derived(`produck_framework_instances_v1_ws_${data.workspaceId}_proj_${projectId}`);
  ...
  function handleKeydown(e: KeyboardEvent) {
    if (e.key === 'f' && !e.ctrlKey && !e.metaKey && !e.altKey && view === 'editor' && selectedInstance) {
  ...
  ] as Array<'all' | FrameworkTemplate['category']>);
  ...
  function createInstance(template: FrameworkTemplate) {
    ...
    if (template.id === 'interview-snapshot') {
      emptyValues.snapshots = JSON.stringify([{
        id: crypto.randomUUID(), personName: '', personRole: '', personPhoto: '',
        interviewDate: new Date().toISOString().slice(0, 10), quote: '', quickFacts: '',
        insights: '', opportunities: '', transcript: ''
      }]);
      ...
      emptyValues.storyMap = JSON.stringify({ product: '', actors: [], levels: 2, activities: [], stories: { 'must-have': [], performance: [], delighter: [] } });
  ...
  $effect(() => {
    ...
    if (savedView) {
      try {
        ...
        if (vs.view === 'editor' && vs.instanceId && instances.some((i) => i.id === vs.instanceId)) {
          view = vs.view;
  ...
  let EditorComponent = $derived(selectedTemplate ? CUSTOM_EDITORS[selectedTemplate.id] ?? null : null);
</script>
...
<div class="space-y-5">
  {#if !fullscreen}
    <header>
      <div>
        ...
        <p class="mt-0.5 text-sm text-cork-500">Pick lightweight templates and fill them for the active workspace.</p>
      </div>
    </header>
  {/if}
  ...
  {:else}
    ...
    <section class="min-w-0">
      {#if view === 'templates'}
        ...
        <div class="grid grid-cols-4 gap-3">
          {#each filteredTemplates as template (template.id)}
            ...
            {@const CatIcon = template.category === 'Discovery' ? Search : template.category === 'Strategy' ? Compass : template.category === 'Validation' ? FlaskConical : template.category === 'Delivery' ? Rocket : Target}
            {@const added = existingTemplateIds.has(template.id)}
            <article class="rounded-lg border border-cork-300/50 bg-cork-50/50 shadow-sm transition-all hover:shadow-md">
              <!-- Header: title left, category icon right -->
              <div class="flex items-center justify-between gap-2 px-3 py-2">
                <h2 class="truncate font-display text-sm font-semibold {added ? 'text-cork-400' : 'text-cork-800'} pr-2">{template.name}</h2>
                <CatIcon class="size-3.5 shrink-0 {added ? 'text-cork-300' : 'text-cork-400'}" />
              </div>
              <!-- Cover: icon → + on hover -->
              <button
                type="button"
                class="relative flex aspect-[3/2] w-full cursor-pointer items-center justify-center bg-cork-200/50 group {added ? 'opacity-60' : ''}"
                onclick={() => (added ? openDraft(instances.find((i) => i.templateId === template.id)!.id) : createInstance(template))}
              >
                <Icon class="size-12 text-cork-700/70 transition-opacity group-hover:opacity-0" />
                <Plus class="absolute size-10 text-cork-700 opacity-0 transition-opacity group-hover:opacity-100" />
              </button>
              <!-- Description -->
              <div class="px-3 pb-3 pt-2">
                <p class="line-clamp-2 text-[11px] leading-snug {added ? 'text-cork-400' : 'text-cork-500'}">{template.description}</p>
              </div>
            </article>
          {/each}
        </div>
              ...
              {#each selectedTemplate.fields as field (field.id)}
                ...
                <label class="block">
                  <div class="mb-1.5 flex items-center gap-1.5">
                    {#if filled}<CheckCircle2 class="size-3.5 text-green-600" />{:else}<Circle class="size-3.5 text-cork-300" />{/if}
      ...
      {:else}
        <div class="flex min-h-80 flex-col items-center justify-center rounded-xl border border-cork-300/50 bg-cork-50/50 px-4 text-center">
    ...
    <!-- Draft sidebar -->
    <aside class="h-fit rounded-xl border border-cork-300/50 bg-cork-50/60 p-2.5 shadow-sm lg:sticky lg:top-20">
      <div>
        <div class="space-y-1">
          <button type="button" class="flex w-full cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-medium transition-colors {view === 'templates' ? 'bg-cork-700 text-cork-50 shadow-sm' : 'text-cork-700 hover:bg-cork-200/60 hover:text-cork-900'}" onclick={() => (view = 'templates')}>
        ...
        {:else}
          <div class="space-y-1 pr-1">
            {#each paginatedInstances as instance (instance.id)}
              <div class="group rounded-lg px-2 py-1.5 transition-colors {selectedInstance?.id === instance.id && view === 'editor' ? 'bg-cork-200/90 text-cork-900 shadow-sm' : 'text-cork-600 hover:bg-cork-200/50 hover:text-cork-800'}">
                <div class="flex items-center gap-1.5">
                  <button type="button" class="min-w-0 flex-1 cursor-pointer text-left" onclick={() => openDraft(instance.id)}>
                    <span class="block truncate text-xs font-medium leading-tight text-current">{instance.title}</span>
                  ...
                  <button type="button" class="flex size-6 shrink-0 cursor-pointer items-center justify-center rounded text-cork-300 opacity-0 transition-all hover:bg-red-50 hover:text-red-500 group-hover:opacity-100" title="Delete draft" onclick={() => confirmDelete(instance.id)}>
          ...
          {#if totalDraftPages > 1}
            <div class="mt-2 flex items-center justify-between gap-1 border-t border-cork-300/40 pt-2">
              <button
                type="button"
                class="cursor-pointer rounded p-0.5 text-cork-500 transition-colors hover:bg-cork-200/50 hover:text-cork-700 disabled:opacity-30 disabled:cursor-default"
...
<Dialog.Root open={showHelp} onOpenChange={(open) => { if (!open) showHelp = false; }}>
  <Dialog.Content class="border-cork-300 bg-cork-50 text-cork-800 sm:max-w-lg">
    ...
    <div class="space-y-4 text-sm">
      {#if selectedTemplate?.instructions}
        <div>
          <h4 class="mb-1.5 text-xs font-semibold tracking-wide text-cork-500 uppercase">How to Use</h4>
          <p class="leading-relaxed text-cork-700">{selectedTemplate.instructions}</p>
        </div>
      {/if}
      {#if selectedTemplate?.terminology}
        <div>
          <h4 class="mb-1.5 text-xs font-semibold tracking-wide text-cork-500 uppercase">Terminology</h4>
