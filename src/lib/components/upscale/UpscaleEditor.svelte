...
>
  <!-- Left: Canvas -->
  <div class="flex min-w-0 flex-col items-start justify-start gap-4">
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div
      bind:this={containerEl}
      class="relative max-h-full w-fit overflow-hidden rounded-xl select-none"
    ...
    >
      ...
      {#if !processing && origWidth > 0}
        <div
          class="pointer-events-none absolute top-0 bottom-0 z-10 w-0.5 bg-cork-600 shadow-md"
          style="left: {sliderPos}%;"
        >
          <div
            class="absolute top-1/2 left-1/2 flex h-8 w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-cork-600 shadow-lg"
          >
            <span class="text-xs font-bold text-cork-100">&#x2194;</span>
          </div>
        </div>
        <div
          class="pointer-events-none absolute top-2 left-2 z-10 rounded bg-cork-800/70 px-1.5 py-0.5 text-[10px] font-medium text-cork-50"
        >
          Before
        </div>
        <div
          class="pointer-events-none absolute top-2 right-2 z-10 rounded bg-cork-800/70 px-1.5 py-0.5 text-[10px] font-medium text-cork-50"
        >
          After
        </div>
      {/if}
      ...
      {#if origWidth > 0 && origHeight > 0}
        <img
          ...
          alt=""
          class="invisible block max-h-[50vh] max-w-full md:max-h-[calc(100vh-260px)]"
        />
      {:else}
  ...
  >
    <div>
      <p class="mb-2 text-xs font-semibold tracking-wider text-cork-500 uppercase">Dimensions</p>
      <div class="space-y-1.5 text-sm">
        ...
        <div class="flex justify-between">
          ...
          <span class="font-medium text-cork-700"
            >{resultWidth > 0 ? `${resultWidth} x ${resultHeight}` : '—'}</span
          >
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="space-y-1.5 text-sm">
        ...
        <div class="flex justify-between">
          <span class="text-cork-500">Enhanced</span>
          <span class="font-medium text-cork-700"
            >{resultFileSize > 0 ? formatSize(resultFileSize) : '—'}</span
          >
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="flex gap-1.5">
        {#each formats as f (f.key)}
          <button
            type="button"
            class="flex-1 rounded-md py-1.5 text-xs font-medium transition-colors {format === f.key
