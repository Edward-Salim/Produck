<script lang="ts">
  ...
  let allACDone = $derived(
    !!story?.details &&
      ...
      (story.checkedAcs?.length ?? 0) >= story.details.length
  );
  ...
  >
    {#if story}
      <Dialog.Header class="gap-1">
        ...
        <Dialog.Title
          class="text-lg font-bold text-cork-800 {allACDone ? 'text-cork-400 line-through' : ''}"
        >
          {story.title}
        </Dialog.Title>
      </Dialog.Header>
      ...
      <div class="flex flex-col gap-4">
        ...
        {#if story.details && story.details.length > 0}
          <div>
            <p class="mb-2 text-[10px] font-bold tracking-widest text-cork-500">
              ACCEPTANCE CRITERIA
            </p>
            <ul class="space-y-1.5">
              {#each story.details as detail, i (detail)}
                ...
                <li class="flex items-start gap-2">
                  ...
                  <span class="text-sm {checked ? 'text-cork-400 line-through' : 'text-cork-700'}"
                    >{detail}</span
                  >
                </li>
              {/each}
