<script lang="ts">
  ...
  let activeTypes = $state(
    new SvelteSet<string>(browser ? JSON.parse(localStorage.getItem('kb_types') || '[]') : [])
  );
  ...
  function typeLabel(key: string): string {
    return (
      { bug: 'Bug', feature: 'Feature', improvement: 'Improvement', task: 'Chore' }[key] ?? key
    );
  }
  ...
  async function onDrop(e: DragEvent, toColId: string) {
    ...
    columns = columns.map((c) => {
      if (c.id === draggingFromColId)
        return { ...c, cards: c.cards.filter((card) => card.id !== draggingCardId) };
      if (c.id === toColId) {
...
{#if loading}
  <div style="height: calc(100vh - 96px);" class="flex flex-col overflow-hidden">
    <div class="flex flex-1 gap-4 overflow-x-auto px-1 pb-2 [scrollbar-width:none]">
      {#each DEFAULT_COLUMNS as col}
        <div class="flex w-60 shrink-0 flex-col rounded-xl" style="background: {col.color};">
          <div class="rounded-t-xl bg-black/5 px-2.5 py-2">
          ...
          <div class="flex-1 space-y-1.5 p-2.5">
            {#each Array(col.id === 'col-todo' ? 4 : col.id === 'col-progress' ? 2 : col.id === 'col-done' ? 3 : 1) as _}
              <div
                class="animate-pulse rounded-r-lg bg-white px-2.5 py-1.5"
                style="box-shadow: 0 1px 3px rgba(0,0,0,.06);border-left: 3px solid {col.id ===
                'col-blocked'
                  ? '#fca5a5'
                  : col.id === 'col-done'
                    ? '#86efac'
...
{:else if columns.length === 0}
  <EmptyState
    icon={Columns3}
    title="No columns"
    description="Add cards via the database to populate this board"
  />
{:else}
  <div style="height: calc(100vh - 96px);" class="flex flex-col overflow-hidden">
    <!-- Type filter -->
        ...
        {#if sortOpen}
          ...
          >
            {#each SORT_OPTIONS as opt}
              <button
                class="block w-full cursor-pointer px-3 py-1 text-left text-[10px] whitespace-nowrap {sortMode ===
                opt.key
                  ? 'bg-cork-100 font-medium text-cork-800'
                  : 'text-cork-600 hover:bg-cork-50'}"
                onclick={() => {
                  sortMode = opt.key;
                  sortOpen = false;
                }}>{opt.label}</button
    ...
    <div class="flex flex-1 gap-4 overflow-x-auto px-1 pb-2 [scrollbar-width:none]">
      {#each columns as col (col.id)}
        ...
        <div
          class="flex w-60 shrink-0 flex-col rounded-xl transition-all {dropTargetColId ===
            col.id && draggingCardId !== null
            ? 'scale-[1.01]'
            : ''}"
        ...
        >
          <!-- Column header -->
          <div class="rounded-t-xl px-2.5 py-2 {dark ? 'bg-white/10' : 'bg-black/5'}">
            <div class="flex items-center gap-2">
              <h2 class="text-sm font-bold {dark ? 'text-white/90' : 'text-gray-700'}">
                {col.title}
              </h2>
              ...
              {#if wipExceeded}
                <span
                  class="shrink-0 rounded-full bg-red-500 px-1.5 py-0.5 text-[8px] font-bold text-white"
                  title="WIP limit exceeded">!</span
                >
              {/if}
            </div>
          </div>
          ...
          <div class="relative min-h-0 flex-1">
            ...
            >
                ...
                >
                  ...
                  <div class="mt-1 flex items-center gap-1.5 text-[9px]">
                    {#if col.id === 'col-blocked'}
                      <button
                        type="button"
                        class="shrink-0 cursor-pointer rounded p-0.5 {card.blockReason
                          ? 'text-red-500 hover:text-red-600'
                          : 'text-gray-300 hover:text-red-400'}"
                        title={card.blockReason
                          ? `Blocked by ${card.blockedBy || '?'}: ${card.blockReason}`
                          : 'Add block reason'}
                        onclick={(e) => {
                          e.stopPropagation();
                          openBlockModal(card);
              ...
              {#if col.cards.length === 0}
                <p
                  class="py-4 text-center text-[10px] italic {dark
                    ? 'text-white/40'
                    : 'text-gray-400'}"
                >
                  No cards
            ...
            {#if showColumnFade[col.id]}
              <div
                class="pointer-events-none absolute inset-x-0 bottom-0 h-8 rounded-b-xl {dark
                  ? ''
...
{#if blockModalCardId}
  ...
  <div class="pointer-events-none fixed inset-0 z-50 flex items-center justify-center p-4">
    <div
      class="pointer-events-auto w-full max-w-sm rounded-xl border border-cork-300 bg-white p-5 shadow-xl"
    >
      {#if blockedCard}
      ...
      <div class="mt-3 space-y-3">
        <label class="block">
          ...
          <div class="relative mt-1">
            <button
              type="button"
              class="block w-full cursor-pointer rounded-lg border border-cork-300 bg-cork-50/50 px-3 py-1.5 text-left text-sm {selectedBlocker
                ? 'text-cork-800'
                : 'text-cork-400'}"
              onclick={() => {
                blockDropdownOpen = !blockDropdownOpen;
                blockSearch = '';
              }}
            >
              {#if selectedBlocker}
                <span class="font-mono text-[10px] text-cork-400">{selectedBlocker.id}</span>
            ...
            {#if blockDropdownOpen}
              ...
              ></span>
              <div
                class="absolute top-full left-0 z-[70] mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg"
              >
                <div class="flex items-center gap-1.5 border-b border-gray-100 px-2 py-1.5">
                  <Search class="size-3 shrink-0 text-cork-400" />
                  <input
                    type="text"
                    class="block w-full border-0 bg-transparent py-0.5 text-xs text-cork-800 outline-none focus:ring-0 focus:outline-none"
                ...
                <div class="max-h-36 overflow-y-auto py-1">
                  {#each filteredBlockableCards as c}
                    <button
                      type="button"
                      class="block w-full cursor-pointer px-3 py-1.5 text-left text-xs {blockedByInput ===
                      c.id
                        ? 'bg-cork-100 font-medium text-cork-800'
                        : 'text-cork-600 hover:bg-cork-50'}"
...
{#if showHistory}
  ...
  <div class="pointer-events-none fixed inset-0 z-50 flex items-center justify-center p-4">
    <div
      class="pointer-events-auto w-full max-w-md rounded-xl border border-cork-300 bg-white p-5 shadow-xl"
    >
      <div class="flex items-center justify-between">
        <h3 class="font-display text-sm font-semibold text-cork-800">Activity</h3>
        <button
          type="button"
          class="cursor-pointer rounded p-0.5 text-cork-400 hover:text-cork-600"
          onclick={() => {
            showHistory = false;
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-0.5 text-[10px] text-cork-500">
                    {#if entry.action === 'move'}
                      Moved from <span class="font-medium text-cork-600">{entry.fromValue}</span> to
                      <span class="font-medium text-cork-600">{entry.toValue}</span>
                    {:else if entry.action === 'assign'}
                      ...
                      {:else if entry.toValue === 'unassigned'}
                        Unassigned (was <span class="font-medium text-cork-600"
                          >{entry.fromValue}</span
                        >)
                      {:else}
                        Reassigned from <span class="font-medium text-cork-600"
                          >{entry.fromValue}</span
                        >
                        to <span class="font-medium text-cork-600">{entry.toValue}</span>
                      {/if}
                    {/if}
                  </p>
                  <p class="mt-0.5 flex items-center gap-1 text-[9px] text-cork-400">
                    <span
                      class="inline-flex size-4 shrink-0 items-center justify-center rounded-full text-[7px] font-semibold text-white"
                      style="background: {assigneeColor(entry.actor)};"
                      >{actorInitial(entry.actor)}</span
                    >
