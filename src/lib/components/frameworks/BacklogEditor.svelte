<script lang="ts">
  ...
  function addEpic() {
    ...
    epics = [
      ...epics,
      { code: newEpicCode.trim(), title: newEpicTitle.trim(), actors: [], stories: [] }
    ];
    newEpicCode = '';
    newEpicTitle = '';
    save();
...
{#if draftMode === 'edit'}
  <div class="mb-3 flex items-center gap-2">
    <input
      bind:value={newEpicCode}
      placeholder="Epic code (e.g. A1)"
      class="w-16 rounded border border-cork-300/50 bg-white px-2 py-1 text-xs text-cork-800 outline-none"
    />
    <input
      bind:value={newEpicTitle}
      placeholder="Epic title"
      class="flex-1 rounded border border-cork-300/50 bg-white px-2 py-1 text-xs text-cork-800 outline-none"
    />
    <button
      type="button"
      class="cursor-pointer rounded bg-cork-700 px-2.5 py-1 text-xs font-medium text-cork-50 hover:bg-cork-800"
      onclick={addEpic}
    >
      <Plus class="inline size-3" /> Epic
    </button>
  </div>
{/if}
...
{#if localEpics.length === 0}
  <EmptyState
    ...
    description="Add epics and tickets via the database"
  />
{:else}
      ...
      >
        ...
        {#if draftMode === 'edit'}
          <button
            ...
            onclick={() => addStory(epic.code)}><Plus class="size-3" /></button
          >
          <button
            type="button"
            class="cursor-pointer text-xs text-cork-300 hover:text-red-500"
            onclick={() => removeEpic(epic.code)}><Trash2 class="size-3" /></button
          >
        {/if}
      </div>
        ...
        >
          <div class="flex min-w-0 items-center gap-1.5 md:gap-2">
            ...
            {#if draftMode === 'edit'}
              <input
                value={story.id}
                class="w-12 shrink-0 rounded border border-cork-300/50 bg-white px-1 py-0 font-mono text-[10px] text-cork-500 outline-none"
                oninput={(e) => updateStoryField(story.id, 'id', e.currentTarget.value)}
              />
              <input
                value={story.title}
                class="min-w-0 flex-1 rounded border border-cork-300/50 bg-white px-1 py-0 text-sm text-cork-800 outline-none"
                oninput={(e) => updateStoryField(story.id, 'title', e.currentTarget.value)}
              />
            {:else}
              ...
              >
            {/if}
            {#if story.task}<span class="shrink-0 text-[10px] text-cork-400">({story.task})</span
              >{/if}
          </div>
                ...
                {#each allPics as p (p)}
                  <button
                    ...
                    story.pic
                      ...
                      : ''}"
                    onclick={() => updatePic(story, p)}>{p}</button
                  >
                {/each}
              </div>
            {/if}
          ...
          <div class="flex items-center justify-center gap-1">
            {#if isDone}<CircleCheck class="size-5 text-cork-500" />{:else}<Circle
                class="size-5 text-cork-300"
              />{/if}
            {#if draftMode === 'edit'}
              <button
                type="button"
          ...
          {#each story.acceptanceCriteria as ac, i (i)}
            ...
            >
              <div class="flex min-w-0 items-center gap-2 pl-5 md:pl-8">
                ...
                >
                  {#if checked}<SquareCheck class="size-4 text-green-600" />{:else}<Square
                      class="size-4 text-cork-400"
                    />{/if}
                </button>
              ...
              <div class="flex items-center justify-end">
                {#if checked && getACTime(story, i)}<span class="text-[9px] text-cork-400"
                    >{getACTime(story, i)}</span
                  >{/if}
              </div>
            </div>
          {/each}
