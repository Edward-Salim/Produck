...
4. After import, always audit:
   ...
   - section ids and labels are named cleanly: if there is only one distinct `verse`, `chorus`, `bridge`, or `outro`, use unnumbered ids/labels like `verse` / `Verse` and `chorus` / `Chorus`; use numbered variants like `verse-1`, `verse-2`, `chorus-1`, `chorus-2` only when those sections are genuinely different. Never use alphabetic section suffixes such as `chorus-1a`, `chorus-1b`, `Chorus 1a`, or `Chorus 1B`; assign distinct sections consecutive whole numbers instead. Repeat sections should point to the clean target id, for example `repeat-chorus` with `repeatOf: "chorus"` when there is only one chorus. If the same target repeats back-to-back, collapse it into one repeat marker with an occurrence suffix, for example `repeat-chorus-2x` / `Repeat Chorus 2x` with `repeatOf: "chorus"`. If the same target repeats in separate non-consecutive places, keep repeat ids unique as needed, such as `repeat-chorus-1` and `repeat-chorus-2`, but keep the label tied to the target section unless the marker itself means repeated back-to-back.
